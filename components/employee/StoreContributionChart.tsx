...
interface Props {
  employeeName: string;
  employeeMonthlySales: number;
  storeMonthlySales: number;
...
const TONE_STYLES = {
  gold:    { bg: 'bg-amber-500/15',   text: 'text-amber-200',   iconBg: 'bg-amber-400/30' },
  emerald: { bg: 'bg-emerald-500/15', text: 'text-emerald-200', iconBg: 'bg-emerald-400/30' },
  sky:     { bg: 'bg-sky-500/15',     text: 'text-sky-200',     iconBg: 'bg-sky-400/30' },
  amber:   { bg: 'bg-amber-500/10',   text: 'text-amber-100',   iconBg: 'bg-amber-400/20' },
} as const;
...
function getMotivation(pct: number, sales: number): Motivation {
  if (sales === 0) {
    return { headline: 'Mulai hari ini', detail: 'Setiap transaksi membuat kontribusimu naik.', Icon: Flame, tone: 'amber' };
  }
  if (pct >= 25) {
    return { headline: 'Top kontributor!', detail: 'Kamu salah satu penggerak utama toko bulan ini.', Icon: Trophy, tone: 'gold' };
  }
  if (pct >= 15) {
    return { headline: 'Momentum bagus', detail: 'Sedikit lagi untuk masuk top kontributor toko.', Icon: TrendingUp, tone: 'emerald' };
  }
  if (pct >= 8) {
    return { headline: 'Terus naik', detail: 'Tambah beberapa transaksi lagi untuk dorong kontribusimu.', Icon: Target, tone: 'sky' };
  }
  return { headline: 'Tetap semangat', detail: 'Setiap penjualanmu menambah kontribusi ke toko.', Icon: Flame, tone: 'amber' };
}
...
export function StoreContributionChart({
  ...
  // Animate the arc + bar filling in on first paint
  const [animatedPct, setAnimatedPct] = useState(0);

  useEffect(() => {
    const t = setTimeout(() => setAnimatedPct(contributionPct), 80);
    return () => clearTimeout(t);
  }, [contributionPct]);

  const motivation        = getMotivation(contributionPct, employeeMonthlySales);
  const tone              = TONE_STYLES[motivation.tone];
  const employeeFilled    = DONUT_CIRCUMFERENCE * (Math.min(100, animatedPct) / 100);
  const othersPct         = Math.max(0, 100 - contributionPct);
  const storeShareOthers  = Math.max(0, storeMonthlySales - employeeMonthlySales);
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-white/[0.10] via-white/[0.06] to-transparent p-5">
      ...
      <div className="relative mb-4 flex items-start justify-between gap-3">
        ...
        <div className={cn('flex shrink-0 items-center gap-1.5 rounded-full px-2.5 py-1', tone.bg)}>
      ...
      {/* ── Donut ──────────────────────────────────────────────────────── */}
      <div className="relative mx-auto flex items-center justify-center" style={{ width: DONUT_SIZE, height: DONUT_SIZE }}>
        <svg
          width={DONUT_SIZE}
          height={DONUT_SIZE}
        ...
        >
          <defs>
            <linearGradient id="contribArc" x1="0%" y1="0%" x2="100%" y2="100%">
              <stop offset="0%"   stopColor="#fbbf24" />
              <stop offset="100%" stopColor="#f59e0b" />
            </linearGradient>
          </defs>

          {/* Track — represents teammates' share */}
          <circle
            cx={DONUT_SIZE / 2}
            cy={DONUT_SIZE / 2}
            r={DONUT_RADIUS}
        ...
        {/* Centre figure — one thing only: the contribution % and the absolute Rp */}
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
          ...
          <p className="mt-2 text-[11px] font-semibold text-amber-200 tabular-nums">
            Rp {fmtCompact(employeeMonthlySales)}
          </p>
        </div>
      </div>

      {/* ── Stacked bar (proportion at a glance) ──────────────────────── */}
      <div className="relative mt-5">
        <div className="flex h-2.5 w-full overflow-hidden rounded-full bg-white/[0.08]">
          <div
            ...
            style={{ width: `${Math.min(100, animatedPct)}%`, transition: BAR_TRANSITION }}
          />
        </div>
        {/* Small marker labels on extreme ends */}
      ...
      <div className="relative mt-4 grid grid-cols-5 gap-2">
        {/* Employee — takes 3/5 to assert visual lead */}
        <div className="col-span-3 rounded-2xl border border-amber-400/30 bg-gradient-to-br from-amber-400/15 to-amber-500/5 px-3.5 py-2.5">
          <div className="flex items-center gap-1.5">
            ...
            <p className="text-[10px] font-bold uppercase tracking-wider text-amber-200/80">Kamu</p>
          </div>
          ...
          <p className="text-[10px] text-amber-200/50 tabular-nums">
            {(Math.round(animatedPct * 10) / 10).toLocaleString('id-ID')}% dari total toko
          </p>
        </div>

        {/* Teammates — 2/5, quieter */}
        <div className="col-span-2 rounded-2xl border border-white/10 bg-white/[0.04] px-3 py-2.5">
          <div className="flex items-center gap-1.5">
            ...
            <p className="text-[10px] font-bold uppercase tracking-wider text-primary-foreground/45">Rekan</p>
          </div>
      ...
      {/* ── Coach line ────────────────────────────────────────────────── */}
      <div className={cn('relative mt-3 flex items-start gap-2.5 rounded-2xl px-3.5 py-2.5', tone.bg)}>
        <div className={cn('flex h-7 w-7 shrink-0 items-center justify-center rounded-xl', tone.iconBg)}>
          <motivation.Icon className={cn('h-3.5 w-3.5', tone.text)} strokeWidth={2.5} />
        </div>
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-[10px] font-medium text-primary-foreground/30 tabular-nums">
            Total toko Rp {fmtCompact(storeMonthlySales)}
          </p>
        </div>
      </div>
    </div>
