'use client';
// components/shared/AccountNotificationBell.tsx
//
// Role-agnostic notification bell for panels that have no bell of their own
// (Finance, Audit). Same polling / read-state behaviour as
// components/ops/layout/NotificationBell.tsx, but talks to the shared
// /api/account/notifications endpoint.
...
export default function AccountNotificationBell() {
  ...
  async function handleSelect(item: NotificationRow) {
    ...
    if (!item.isRead) {
      ...
      void fetch(`/api/account/notifications/${item.id}`, { method: 'PATCH' });
    }
