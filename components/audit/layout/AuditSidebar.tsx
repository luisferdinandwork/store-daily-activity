'use client';
// components/audit/layout/AuditSidebar.tsx
//
// Collapsible sidebar for the Audit panel.
// Mirrors the structure of FinanceSidebar, amber-accented:
//
//   Overview
//     Dashboard         /audit
//
//   Issues
//     Issues            /audit/issues        (issues routed to Audit role)
...
const NAV: NavSection[] = [
  {
    section: 'Overview',
    items: [
      { href: '/audit', label: 'Dashboard', icon: LayoutDashboard, exact: true },
    ],
  },
  {
    section: 'Issues',
...
export default function AuditSidebar({ collapsed = false, userName = 'Audit' }: Props) {
    ...
    <TooltipProvider delayDuration={120}>
      ...
      >
        ...
        <div className="flex items-center gap-2 px-3 py-5 min-w-0">
          {collapsed ? (
            <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-amber-600">
          ...
          {!collapsed && (
            <div className="min-w-0 overflow-hidden">
              <p className="truncate text-xs font-semibold text-foreground">Audit Panel</p>
            ...
            {collapsed ? (
              <NavTooltip label={`${displayName} · Logout`}>
                <button
                  ...
                  className="flex h-7 w-7 items-center justify-center rounded-full bg-amber-50 text-xs font-semibold text-amber-700 hover:bg-red-50 hover:text-red-600 transition-colors"
                >
                  {initial}
                </button>
              </NavTooltip>
