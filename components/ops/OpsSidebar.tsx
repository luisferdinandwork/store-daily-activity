...
export default function OpsSidebar({ storeName = 'Store Manager' }: Props) {
  ...
  const [tasksOpen, setTasksOpen] = useState(pathname.startsWith('/ops/tasks'));
  const [pendingCounts, setPendingCounts] = useState<Record<string, number>>({});

  const taskSectionActive = pathname.startsWith('/ops/tasks');
  ...
  useEffect(() => {
    ...
    async function fetchCounts() {
      try {
        // Get today's date in local YYYY-MM-DD format (e.g., "2024-05-20")
    ...
    // Refresh every 5 minutes so the numbers stay relatively accurate
    // without putting too much load on the database
    ...
    <aside className="flex h-screen w-64 flex-col border-r border-border bg-card">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4">
        ...
        <div className="mb-5">
          ...
          {tasksOpen && (
            <ul className="mt-1 space-y-0.5 border-l border-border/70 pl-2 ml-3">
