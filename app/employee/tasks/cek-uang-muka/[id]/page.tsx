'use client';
// app/employee/tasks/cek-uang-muka/[id]/page.tsx
...
interface CekUangMukaDenomination {
  id?: string;
  taskId?: string;
  userId?: string;
  storeId?: string;
...
const MAX_UANG_MUKA_TOTAL = 500_000;
...
function buildDenominationRows(
  existing: CekUangMukaDenomination[] | undefined,
): CekUangMukaDenomination[] {
  const byValue = new Map<number, CekUangMukaDenomination>();
...
function payloadRows(rows: CekUangMukaDenomination[]) {
  return rows.map((row) => ({
...
function totalRows(rows: CekUangMukaDenomination[]) {
  return rows.reduce(
...
export default function CekUangMukaDetailPage() {
  ...
  const [taskData, setTaskData] = useState<CekUangMukaData | null>(null);
  const [rows, setRows] = useState<CekUangMukaDenomination[]>(buildDenominationRows([]));
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = (await res.json()) as {
        tasks?: Array<{ type: string; data: CekUangMukaData }>;
      };

      const found = data.tasks?.find(
        (task) => task.type === 'cek_uang_muka' && task.data.id === taskId,
      );
      ...
      console.error('[CekUangMukaDetailPage] load error:', e);
  ...
  const {
    ...
    url: '/api/employee/tasks/cek-uang-muka',
  ...
  const maxAmount = parseMoney(taskData?.maxAmount, MAX_UANG_MUKA_TOTAL);
  ...
  const submitHint = (() => {
    ...
    if (isOverLimit) return `Total uang muka maksimal ${formatRupiah(maxAmount)}.`;
    return '';
  })();
  ...
  function updateQuantity(denominationValue: number, quantity: number) {
    setRows((prev) => {
      ...
      if (quantity > maxQtyForRow) {
        toast.error(`Maksimal uang muka ${formatRupiah(maxAmount)}. Qty pecahan ini dibatasi ke ${maxQtyForRow}.`);
      }
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch('/api/employee/tasks/cek-uang-muka', {
        method: 'POST',
      ...
      toast.success(`Cek Uang Muka selesai · ${formatRupiah(totalAmount)} ✓`, {
        duration: 4000,
      });
      router.back();
    } catch (e) {
        ...
        {loading ? (
          ...
          <div className="relative">
            ...
            <div className="space-y-6">
              <Section title="Ringkasan Uang Muka">
                ...
                {isOverLimit && (
                  <div className="mt-3 rounded-2xl border border-red-200 bg-red-50 px-4 py-3">
                    <p className="text-xs font-bold text-red-700">Melebihi limit uang muka</p>
                    <p className="mt-0.5 text-xs text-red-600">
              ...
              {readonly && (
                <div className="flex items-center justify-center gap-2 rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-green-700">
                  ...
                  <p className="text-xs font-semibold">
                    Data pecahan uang muka sudah tersimpan.
                  </p>
                </div>
              )}
            </div>
