...
// ─────────────────────────────────────────────────────────────────────────────
// Dedicated detail page for the Item Dropping task.
//
// Two scenarios:
//   A) No dropping today  → single toggle, no photos required → submit directly.
//   B) Item dropped       → record dropTime + photos, then either:
//       • Confirm receipt (isReceived=true, receiveTime) → status 'completed'.
//       • Submit without receipt → status 'discrepancy' → carry-forward tomorrow.
//
// Carry-forward (discrepancy from prior day):
//   The page detects an existing discrepancy task and surfaces a distinct
//   "Konfirmasi Penerimaan" panel so the new-day employee can quickly confirm
//   receipt without redoing the whole form.
// ─────────────────────────────────────────────────────────────────────────────
...
function OptionButton({
  selected, onClick, icon: Icon, label, description, color, disabled,
}: {
  selected:    boolean;
  onClick:     () => void;
  ...
  const colors = {
    green: {
      ...
      inactive: 'border-border bg-card hover:border-green-200',
    },
    amber: {
      active:   'border-amber-400 bg-amber-50',
      icon:     'bg-amber-100 text-amber-700',
      label:    'text-amber-800',
      inactive: 'border-border bg-card hover:border-amber-200',
    },
  }[color];
  ...
  return (
    ...
    >
      <div className={cn(
        ...
        selected ? colors.icon : 'bg-secondary text-muted-foreground',
      )}>
      ...
      <div className="min-w-0 flex-1 pt-0.5">
        <div className="flex items-center gap-2">
          <p className={cn(
            'text-sm font-bold',
            selected ? colors.label : 'text-foreground',
          )}>{label}</p>
          {selected && (
            <span className={cn(
...
// ─── Carry-forward banner (discrepancy from prior day) ────────────────────────

function CarryForwardBanner({
  task, onConfirmReceipt, geo, scheduleId, storeId, disabled,
}: {
  task:             ItemDroppingData;
  ...
  const dropDate = task.date
    ? new Date(task.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })
    : '–';
  const dropTimeStr = task.dropTime
    ? new Date(task.dropTime).toLocaleString('id-ID', { hour: '2-digit', minute: '2-digit' })
    : '–';
    ...
    <div className="rounded-2xl border-2 border-amber-400 bg-amber-50 overflow-hidden">
      ...
      {expanded && task.droppingPhotos.length > 0 && (
        <div className="px-4 pb-3">
          <p className="mb-2 text-[10px] font-semibold text-amber-700 uppercase tracking-wider">Foto dropping kemarin</p>
          <div className="flex flex-wrap gap-2">
      ...
      {!disabled && (
        <div className="border-t border-amber-200 px-4 py-3">
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-amber-500 px-4 py-3 text-sm font-bold text-white transition-all hover:bg-amber-600 active:scale-[0.98]"
          >
            <PackageCheck className="h-4 w-4" />
            Konfirmasi Penerimaan Item
          </button>
          <p className="mt-1.5 text-center text-[10px] text-amber-600">
            Tekan jika item sudah diterima oleh karyawan toko hari ini
          </p>
        </div>
      )}
    </div>
...
function ConfirmReceiptModal({
  ...
  open:       boolean;
  onClose:    () => void;
  onConfirm:  (receiveTime: string, notes: string) => void;
  submitting: boolean;
}) {
  const now = new Date();
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-md rounded-t-3xl bg-background shadow-xl sm:rounded-3xl max-h-[80vh] flex flex-col"
        onClick={e => e.stopPropagation()}
      >
        <div className="flex items-center justify-between gap-3 border-b border-border px-5 py-4">
          <div>
            <h3 className="text-base font-bold text-foreground">Konfirmasi Penerimaan</h3>
            <p className="mt-0.5 text-xs text-muted-foreground">Catat waktu penerimaan item oleh karyawan toko.</p>
          </div>
        ...
        <div className="flex gap-2 border-t border-border px-5 py-4">
          <button onClick={onClose} className="flex-1 rounded-xl border border-border bg-card px-4 py-3 text-sm font-semibold text-foreground">
            Batal
          </button>
          <button
            onClick={() => onConfirm(receiveTime, notes)}
            disabled={!receiveTime || submitting}
            className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-amber-500 px-4 py-3 text-sm font-bold text-white disabled:opacity-50"
          >
...
export default function ItemDroppingDetailPage() {
  ...
  // Carry-forward prior discrepancy task (different from current taskData)
  ...
  // Form state
  const [hasDropping,      setHasDropping]      = useState<boolean | null>(null); // null = not yet chosen
    ...
    try {
      ...
      if (found) {
        ...
        if (d.dropTime) {
          ...
          setDropTime(new Date(dt.getTime() - dt.getTimezoneOffset() * 60000).toISOString().slice(0, 16));
        }
        if (d.receiveTime) {
          const rt = new Date(d.receiveTime);
          setReceiveTime(new Date(rt.getTime() - rt.getTimezoneOffset() * 60000).toISOString().slice(0, 16));
        }

        // Check if there's a prior-day discrepancy task (parentTaskId chain)
        if (d.status === 'discrepancy' && !d.parentTaskId) {
          setPriorTask(d);
        }
      } else {
        setTaskData(null);
  ...
  const { status: saveStatus, lastSaved, error: saveError, save: autoSave } = useAutoSave({
    url:        '/api/employee/tasks/item-dropping',
    baseBody:   { scheduleId },
    debounceMs: 800,
  });
  ...
  // Scenario A: no dropping — always submittable once selected
  ...
  const scenarioBReady       = hasDropping === true && dropTimeFilled && droppingPhotosFilled;
  ...
  async function handleConfirmReceipt(rcvTime: string, rcvNotes: string) {
    if (!priorTask) return;
        ...
        <div className="min-w-0 flex-1">
          ...
          {taskData && (
            <p className="text-[10px] capitalize text-muted-foreground">
              {taskData.shift} shift · {statusLabel[taskData.status] ?? taskData.status}
            </p>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="relative">
            ...
            <div className="space-y-6">
              ...
              {/* ── CARRY-FORWARD: prior day discrepancy ── */}
              ...
              {/* ── READ-ONLY VIEW (completed / verified / discrepancy detail) ── */}
                  ...
                  <div className="rounded-2xl border border-border bg-card p-4 space-y-3">
                    ...
                    {taskData.hasDropping && (
                      <>
                        <div className="flex items-center justify-between border-t border-border pt-3">
                          ...
                          <span className="text-xs font-semibold text-foreground">{fmt(taskData.dropTime)}</span>
                        </div>
                        <div className="flex items-center justify-between border-t border-border pt-3">
                          <span className="text-xs text-muted-foreground">Status Penerimaan</span>
                          <span className={cn(
                            'rounded-full px-2.5 py-0.5 text-[11px] font-bold',
                            taskData.isReceived ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700',
                          )}>
                        ...
                        {taskData.isReceived && (
                          <div className="flex items-center justify-between border-t border-border pt-3">
                            ...
                            <span className="text-xs font-semibold text-foreground">{fmt(taskData.receiveTime)}</span>
                          </div>
                        )}
                    ...
                    <div className="border-t border-border pt-3">
                      ...
                      {taskData.completedAt && (
                        <p className="text-[10px] text-muted-foreground">Selesai: {fmt(taskData.completedAt)}</p>
                      )}
                    </div>
                  </div>
                </Section>
                ...
                <>
                  ...
                  <Section title="Ada Item Dropping Hari Ini?">
                    <div className="space-y-2.5">
                      <OptionButton
                        selected={hasDropping === false}
                        onClick={() => {
                          setHasDropping(false);
                          autoSave({ hasDropping: false });
                        }}
                        icon={PackageCheck}
                        label="Tidak Ada Dropping"
                        description="Tidak ada pengiriman barang hari ini. Task langsung selesai."
                        color="green"
                        disabled={dis}
                      />
                      <OptionButton
                        selected={hasDropping === true}
                        onClick={() => {
                          setHasDropping(true);
                          autoSave({ hasDropping: true });
                        }}
                        icon={PackageOpen}
                        label="Ada Item Dropping"
                  ...
                  {/* Dropping details (shown only when hasDropping=true) */}
                  {hasDropping === true && (
                    <>
                      <Section title="Detail Dropping">
                        <div className="space-y-4">
                          <TimeField
                            label="Waktu Dropping"
                            hint="Waktu saat barang tiba / dikirim ke toko."
                            value={dropTime}
                            onChange={v => {
                              setDropTime(v);
                              autoSave({ dropTime: v ? new Date(v).toISOString() : null });
                            }}
                            disabled={dis}
                          />
                          ...
                          <PhotoUploader
                            ...
                            photos={droppingPhotos}
                            min={PHOTO_RULES.dropping.min}
                            max={PHOTO_RULES.dropping.max}
                            disabled={dis}
                            hint="Foto barang yang tiba di toko sebagai bukti dropping."
                            onChange={urls => {
                              setDroppingPhotos(urls);
                              autoSave({ droppingPhotos: urls }, { immediate: true });
                            }}
                          />
                        </div>
                      </Section>

                      {/* Receipt confirmation (optional at submit time) */}
                      <Section title="Penerimaan Barang">
                        <div className={cn(
                          'rounded-2xl border-2 p-4 space-y-4 transition-all',
                          isReceived ? 'border-green-300 bg-green-50' : 'border-border bg-card',
                        )}>
                          <button
                            type="button"
                            onClick={() => {
                              ...
                              setIsReceived(next);
                              if (!next) setReceiveTime('');
                              autoSave({ isReceived: next, receiveTime: next ? receiveTime || null : null });
                            }}
                            disabled={dis}
                            className={cn(
                          ...
                          >
                            <div className={cn(
                              ...
                              isReceived ? 'border-green-500 bg-green-500' : 'border-border',
                            )}>
                              {isReceived && <Check className="h-3 w-3 text-white" strokeWidth={3} />}
                            </div>
                            <div className="min-w-0 flex-1">
                              <span className="text-sm font-semibold text-foreground">
                                Barang sudah diterima karyawan toko
                              </span>
                              <p className="mt-0.5 text-[11px] text-muted-foreground">
                                Centang jika barang sudah diverifikasi dan diterima saat ini.
                              </p>
                            </div>
                          </button>
                          ...
                          {isReceived && (
                            <TimeField
                              label="Waktu Penerimaan"
                              hint="Waktu saat karyawan toko menerima dan memeriksa barang."
                              value={receiveTime}
                              onChange={v => {
                                setReceiveTime(v);
                                autoSave({ receiveTime: v ? new Date(v).toISOString() : null });
                              }}
                              disabled={dis}
                            />
                          )}
                          ...
                          {!isReceived && (
                            <div className="flex items-start gap-2.5 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5">
                              ...
                              <p className="text-[11px] text-amber-700">
                                Jika barang belum diterima saat submit, task akan dilanjutkan ke shift besok.
                              </p>
                            </div>
                          )}
                        </div>
