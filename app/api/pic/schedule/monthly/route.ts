...
export async function GET(req: NextRequest) {
  ...
  // getMonthlySchedule already joins the shifts table and returns shiftCode
  // on each entry — no extra per-row query needed here.
  const mappedEntries = rawSchedule.entries.map(entry => ({
    id:       String(entry.id),
    userId:   entry.userId,
    userName: entry.userName,
    userType: entry.userEmployeeType,           // renamed for the frontend
    date:     entry.date,
    shiftId:  entry.shiftId,
    shift:    (entry.shiftCode ?? null) as 'morning' | 'evening' | 'full_day' | null,
    isOff:    entry.isOff,
