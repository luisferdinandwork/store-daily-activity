...
//
// GET — issues routed to the IT role. IT sees every store (no area
// scoping, unlike Ops) — mirrors app/api/ops/issues/route.ts's shape/query,
// just routed by the 'it' role instead of 'ops', with no area filter.
...
export async function GET() {
  try {
    ...
    const assignedRows = await db
      ...
      .where(eq(issueRoleAssignments.roleId, itRole.id));
    ...
    const routingConditions = [eq(issues.assignedToRoleId, itRole.id)];
