// app/api/finance/issues/[id]/route.ts
//
// PATCH — advance a Finance-assigned issue's status:
//   in_review — Finance starts working it (optional; can be set any time
//               from "reported").
//   completed — Finance gives final closure, but ONLY once the reporter has
//               already marked the issue "solved" (employee-only, see
//               /api/employee/issues/[id]) — never the other way around.
//
// Finance sees every store — no area scoping (unlike Ops).
  ...
  try {
    ...
    if (!isValidStatus(requestedStatus)) {
      return NextResponse.json(
        { error: 'Invalid status. Finance can only use in_review or completed.' },
        { status: 400 },
      );
    }
    ...
    if (target.status === 'draft') {
      return NextResponse.json(
        { error: 'This issue is still a draft and has not been sent to Finance yet.' },
        { status: 403 },
      );
    }
    ...
    const [financeAssignment] = await db
      .select({ issueId: issueRoleAssignments.issueId })
      .from(issueRoleAssignments)
      .where(
        and(
          eq(issueRoleAssignments.issueId, issueId),
          eq(issueRoleAssignments.roleId, financeRole.id),
        ),
      )
      .limit(1);

    const routedToFinance = !!financeAssignment || target.assignedToRoleId === financeRole.id;

    if (!routedToFinance) {
      return NextResponse.json(
        { error: 'This issue is not routed to Finance.' },
        { status: 403 },
      );
    }
    ...
    if (requestedStatus === 'completed') {
      await notifyIssueEvent({
        ...
        body: 'Finance confirmed the resolution. Issue closed.',
    ...
    console.error('[PATCH /api/finance/issues/[id]]', err);
