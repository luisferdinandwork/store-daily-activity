// app/api/audit/issues/route.ts
//
// GET — issues routed to the Audit role. Audit sees every store (no area
// scoping) — mirrors app/api/ops/issues/route.ts's shape/query, just routed
// by the 'audit' role instead of 'ops', with no area filter.
...
export async function GET() {
  try {
    ...
    const assignedRows = await db
      ...
      .where(eq(issueRoleAssignments.roleId, auditRole.id));
    ...
    const routingConditions = [eq(issues.assignedToRoleId, auditRole.id)];
    ...
    console.error('[GET /api/audit/issues]', err);
