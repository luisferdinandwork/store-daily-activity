// app/api/ops/impact-visits/[id]/photo/route.ts
//
// POST — upload the on-site proof photo for an On Location impact visit.
// Unlike the virtual screenshot route, the client is expected to use a live
// camera capture (components/shared/PhotoUploadGrid + CameraCapture) rather
// than a file picker, matching the app's existing on-site verification
// convention — enforcement of that is a client-side UX choice, same as
// everywhere else camera-only capture is used in this app.
...
export async function POST(
  ...
  const key = `impact-visits/${visitId}/photo-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`;
  const buffer = Buffer.from(await file.arrayBuffer());
  const url = await uploadToStorage(buffer, key, file.type);

  const [updated] = await db
    .update(impactVisits)
    .set({ visitPhotoUrl: url, updatedAt: new Date() })
