  ...
  try {
    ...
    if (roleCode === 'ops' && allowedStoreIds.length === 0) {
      return NextResponse.json({
        ...
        summary: { stores: 0, totalTasks: 0, completed: 0, unpaidTotal: 0 },
        stores: [],
      });
    }
    ...
    function userInfo(id: string | null | undefined) {
      ...
      return u ? { id: u.id, name: u.name, nik: u.nik } : { id, name: null, nik: null };
    }
    ...
    const totalTasks = storesResult.reduce((sum, store) => sum + store.total, 0);
    const completed = storesResult.reduce((sum, store) => sum + store.completed, 0);
    const unpaidTotal = storesResult.reduce((sum, store) => sum + store.unpaidTotal, 0);
    ...
    console.error('GET /api/ops/tasks/setoran failed:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to load setoran tasks.' },
      { status: 500 },
    );
  }
}
