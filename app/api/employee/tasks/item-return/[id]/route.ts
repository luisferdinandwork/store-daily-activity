// app/api/employee/tasks/item-return/[id]/route.ts
//
// GET  — opens the Item Return task: live-syncs Business Central's Transfer
//        Orders against this store (see syncItemReturnForStore), then
//        returns the task + every entry (open and already-confirmed today)
//        with its linked transfer-order info for display.
// POST — confirms ONE transfer order's pickup (qty counted + courier-signed
...
import { itemReturnTasks, schedules, stores } from '@/lib/db/schema';
import {
...
export async function GET(
  ...
  const [task] = await db.select().from(itemReturnTasks).where(eq(itemReturnTasks.id, taskId)).limit(1);
  ...
  const syncResult = await syncItemReturnForStore(task.storeId, {
  ...
  const [freshTask] = await db.select().from(itemReturnTasks).where(eq(itemReturnTasks.id, taskId)).limit(1);
  const rows = await getItemReturnEntriesWithTransferOrder(taskId);
