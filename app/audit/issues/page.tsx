...
// app/audit/issues/page.tsx — Audit issue inbox (desktop)
//
// Adapted from app/ops/issues/page.tsx: same list + detail-drawer mechanics,
// amber accent (matching AuditSidebar), no area scoping — Audit sees
// every store.
    ...
    <div className="fixed inset-0 z-50 flex" onClick={onClose}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
              ...
              <div className="grid grid-cols-3 gap-2">
                {issue.attachmentUrls.map((url, i) => (
                  <a key={i} href={url} target="_blank" rel="noopener noreferrer"
                    className="group relative block aspect-square overflow-hidden rounded-xl border border-slate-200 bg-slate-100 transition-all hover:border-amber-300 hover:shadow-sm">
          ...
          {/* Berita Acara — read-only for Audit, uploaded by the reporter */}
...
function IssueRow({ issue, onClick }: { issue: AuditIssue; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="group w-full rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm transition-all hover:border-amber-200 hover:shadow-md"
    >
...
export default function AuditIssuesPage() {
  ...
  const [issuesList, setIssuesList] = useState<AuditIssue[]>([]);
  ...
  async function handleAdvance(id: string, next: IssueStatus) {
    setUpdating(true);
    try {
      const res = await fetch(`/api/audit/issues/${id}`, {
  ...
  const statCards = [
    { key: 'all'       as const, label: 'Total',     value: meta.all,       color: '#d97706', Icon: AlertTriangle },
  ...
  if (!isAudit) return (
    <div className="flex min-h-full flex-col items-center justify-center gap-4 bg-slate-50 p-8 text-center">
      ...
      <p className="text-sm text-slate-500">Only Audit users can review issue reports.</p>
    </div>
  );
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-400">
              {issuesList.length} issue{issuesList.length !== 1 ? 's' : ''} routed to Audit
            </p>
          </div>
          <button
            onClick={() => load(true)}
            disabled={refreshing}
            className="flex h-9 items-center gap-1.5 rounded-xl border border-amber-200 bg-amber-50 px-3 text-xs font-bold text-amber-700 transition-colors hover:bg-amber-100 disabled:opacity-60"
          >
            ...
            <div className="min-w-[280px] flex-1">
              ...
              <div className="relative">
                ...
                <select value={storeFilter} onChange={e => { setStoreFilter(e.target.value); setSelected(null); }}
                  className="h-11 w-full appearance-none rounded-xl border border-slate-200 bg-white pl-10 pr-10 text-sm font-semibold text-slate-800 focus:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-100">
          ...
          <div className="flex flex-col items-center gap-4 rounded-2xl border border-dashed border-slate-200 bg-white py-20 text-center">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-slate-400">
                {filter !== 'all' ? `No ${STATUS_CFG[filter as IssueStatus].label.toLowerCase()} issues in scope.` : 'Everything routed to Audit is clear.'}
              </p>
            </div>
          </div>
        ) : (
