...
// app/finance/issues/page.tsx — Finance issue inbox (desktop)
//
// Adapted from app/ops/issues/page.tsx: same list + detail-drawer mechanics,
// emerald accent (matching FinanceSidebar), no area scoping — Finance sees
// every store.
    ...
    <div className="fixed inset-0 z-50 flex" onClick={onClose}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
              ...
              <div className="grid grid-cols-3 gap-2">
                {issue.attachmentUrls.map((url, i) => (
                  <a key={i} href={url} target="_blank" rel="noopener noreferrer"
                    className="group relative block aspect-square overflow-hidden rounded-xl border border-slate-200 bg-slate-100 transition-all hover:border-emerald-300 hover:shadow-sm">
          ...
          {/* Berita Acara — read-only for Finance, uploaded by the reporter */}
...
function IssueRow({ issue, onClick }: { issue: FinanceIssue; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="group w-full rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm transition-all hover:border-emerald-200 hover:shadow-md"
    >
...
export default function FinanceIssuesPage() {
  const { data: session, status: authStatus } = useSession();
  const router = useRouter();
  const role = (session?.user as any)?.role as string | undefined;
  const isFinance = role === 'finance' || role === 'it';

  const [issuesList, setIssuesList] = useState<FinanceIssue[]>([]);
  const [loading,     setLoading]    = useState(true);
  const [refreshing,  setRefreshing] = useState(false);
  const [filter,      setFilter]     = useState<IssueStatus | 'all'>('all');
  const [storeFilter, setStoreFilter]= useState<string>('all');
  const [selected,    setSelected]   = useState<FinanceIssue | null>(null);
  const [updating,    setUpdating]   = useState(false);

  useEffect(() => {
    if (authStatus === 'loading') return;
    if (!session)     { router.replace('/login'); return; }
    if (!isFinance)   router.replace('/');
  }, [authStatus, session, isFinance, router]);

  const load = useCallback(async (isRefresh = false) => {
    isRefresh ? setRefreshing(true) : setLoading(true);
    try {
      const res  = await fetch('/api/finance/issues', { cache: 'no-store' });
  ...
  const statCards = [
    { key: 'all'       as const, label: 'Total',     value: meta.all,       color: '#059669', Icon: AlertTriangle },
  ...
  if (authStatus === 'loading' || !session) return (
    <div className="flex min-h-full items-center justify-center bg-slate-50"><Loader2 className="h-6 w-6 animate-spin text-emerald-400" /></div>
  );
  ...
  if (!isFinance) return (
    <div className="flex min-h-full flex-col items-center justify-center gap-4 bg-slate-50 p-8 text-center">
      ...
      <p className="text-sm text-slate-500">Only Finance users can review issue reports.</p>
    </div>
  );
    ...
    <div className="min-h-full bg-slate-50">
      ...
      <div className="border-b border-slate-200 bg-white px-6 py-5 lg:px-8">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest text-emerald-600">Finance</p>
            <h1 className="text-xl font-bold text-slate-900">Issue Reports</h1>
            <p className="mt-0.5 text-xs text-slate-400">
              {issuesList.length} issue{issuesList.length !== 1 ? 's' : ''} routed to Finance
            </p>
          </div>
          <button
            onClick={() => load(true)}
            disabled={refreshing}
            className="flex h-9 items-center gap-1.5 rounded-xl border border-emerald-200 bg-emerald-50 px-3 text-xs font-bold text-emerald-700 transition-colors hover:bg-emerald-100 disabled:opacity-60"
          >
            ...
            <div className="min-w-[280px] flex-1">
              ...
              <div className="relative">
                ...
                <select value={storeFilter} onChange={e => { setStoreFilter(e.target.value); setSelected(null); }}
                  className="h-11 w-full appearance-none rounded-xl border border-slate-200 bg-white pl-10 pr-10 text-sm font-semibold text-slate-800 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100">
          ...
          <div className="flex flex-col items-center gap-4 rounded-2xl border border-dashed border-slate-200 bg-white py-20 text-center">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-slate-400">
                {filter !== 'all' ? `No ${STATUS_CFG[filter as IssueStatus].label.toLowerCase()} issues in scope.` : 'Everything routed to Finance is clear.'}
              </p>
            </div>
          </div>
        ) : (
